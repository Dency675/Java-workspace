...
public class CustomerService {
	...
	public static void transactionBill(Customer customer) {
		...
		if (customerCode.equals(customer.getCustomerCode())) {
			...
			System.out.print("\nEnter the rate of  transaction:Rs ");
			int rateOfTransaction = scanner.nextInt();

			int currentBalance = (customer.getAccountList().get(choice - 1).getBalance()) - rateOfTransaction;
			customer.getAccountList().get(choice - 1).setBalance(currentBalance);
